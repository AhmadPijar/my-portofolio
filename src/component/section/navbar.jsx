...
const Navbar = () => {
  const { isScroll, menuOpen, setMenuOpen, menuRef, hamburgerRef, toggleHamburger } = useContext(NavbarContext);
  const { theme, setTheme } = useContext(ThemeContext);

  return (
    <header className={`fixed top-0 left-0 w-full z-50  ${isScroll ? 'navbar-scroll' : ''}`}>
      <div className="container relative flex items-center justify-between mx-auto px-10 py-4">
        ...
        <button ref={hamburgerRef} onClick={toggleHamburger} className={`lg:hidden space-y-2 hamburger ${menuOpen ? 'hamburger-active' : ''} `}>
        ...
        <nav className="hidden lg:flex space-x-8">
          <a href="#home" className="text-base text-head dark:text-white font-medium hover:text-primary">
            Home
          </a>
          <a href="#about" className="text-base text-head dark:text-white font-medium hover:text-primary">
            About
          </a>
          <a href="#project" className="text-base text-head dark:text-white font-medium hover:text-primary">
            Project
          </a>
          <a href="#contact" className="text-base text-head dark:text-white font-medium hover:text-primary">
            Contact
          </a>
      ...
      <div
        ref={menuRef}
        className={`absolute top-full right-4 w-64 px-2 py-6 transform ${menuOpen ? 'translate-x-0' : 'translate-x-80'} bg-white rounded-xl shadow-md dark:bg-slate-800 dark:shadow-gray-60 transition-transform duration-300 lg:hidden`}
      >
